...
SELECT ct.name, ct.code, ct.order, ct.color_code, ct.study_guide, sb.name, sb.code, sb.study_guide, crs.name, crs.code, crs.estimated_hours_to_finish, crs.developed_abilities
FROM category ct
         INNER JOIN subcategory sb ON sb.category_id = ct.id
         INNER JOIN course crs ON crs.subcategory_id = sb.id
WHERE ct.code = 'programacao'